...
### 컴퓨터 수 생성
- 1부터 9까지의 서로 다른 수로 이루어진 3자리 수를 생성한다.
...
- 결과를 출력한다.
  - 볼의 경우 "{카운트}볼" 형식으로 출력
  - 스트라이크의 경우 "{카운트}스트라이크" 형식으로 출력
  - 볼, 스트라이크의 경우 {볼 카운트}볼 "{스트라이크 카운트} 스트라이크" 형식으로 출력
  - 낫싱의 경우 "낫싱"을 출력한다.
...
### 게임 재시작
- 종료 후 사용자에게 재시작 유무를 입력 받는다.
- 게임 종료의 경우 1을 입력 받고, 재시작의 경우 2를 입력받는다.
  - 예외
    - 아무것도 입력하지 않은 경우
    - 숫자가 아닌 문자열을 입력한 경우
    - 1, 2가 아닌 다른 숫자를 입력한 경우
- 만약 사용자가 재시작을 원하면 게임을 재시작하고 아니라면 게임을 완전 종료한다.

### 예외 처리
- 예외 상황이 발생하면 IllegalArgumentException을 발생시켜 애플리케이션을 종료한다.
